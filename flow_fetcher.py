...
def get_bist_microstructure_data():
    """TradingView üzerinden ham mikroyapı ve trend verilerini çeker."""
    ...
    payload = {
        ...
        "columns": [
            ...
            "relative_volume_10d_calc", "SMA50", "SMA200", "Perf.1M", "Perf.3M"
        ],
    ...
    try:
        ...
        for item in data.get("data", []):
            ...
            rows.append({
                ...
                "sma50": float(d[9]) if len(d) > 9 and d[9] is not None else 0.0,
                "sma200": float(d[10]) if len(d) > 10 and d[10] is not None else 0.0,
                "perf_1m": float(d[11]) if len(d) > 11 and d[11] is not None else 0.0,
                "perf_3m": float(d[12]) if len(d) > 12 and d[12] is not None else 0.0
            })
...
def fetch_all_data():
    ...
    print(f"[{datetime.now().strftime('%H:%M:%S')}] Ham Emir Akışı ve Trend verileri çekiliyor...")
