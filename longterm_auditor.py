...
def update_signal_lifecycle(df_signals, market_prices, state):
    ...
    for idx, row in df_signals.iterrows():
        ...
        if trailing_stop > prev_stop:
            if peak_gain >= 6.0 and prev_stop <= entry_p:
                exit_alerts.append({
                    "ticker": ticker,
                    "type": "BREAKEVEN_STOP_RAISE",
                    "msg": f"⚡ <b>HIZLI BAŞABAŞ:</b> Hisse +%{peak_gain:.1f} zirve gördü! Aracı kurumunuzdan stopunuzu <b>{trailing_stop:.2f} TL</b> (Maliyet + %1) seviyesine çekerek işlemi <b>TAMAMEN RİSKSİZ</b> hale getirin."
                })
            elif peak_gain >= 14.0:
                exit_alerts.append({
                    "ticker": ticker,
                    "type": "PROFIT_STOP_RAISE",
                    "msg": f"🔒 <b>KÂR KİLİDİ:</b> Hisse +%{peak_gain:.1f} zirve gördü! Stop seviyenizi <b>{trailing_stop:.2f} TL</b> seviyesine yükselterek kârınızı garantiye alın."
                })
...
def run_feedback_loop_optimization(df_signals, state):
    ...
    factors = {
        ...
        "stealth_accumulation": "score_sweep",
        "volume_ignition": "score_ignition"
    }
