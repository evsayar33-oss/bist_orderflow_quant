...
def format_quant_report(df_scored):
    sweepers = df_scored[df_scored['quant_score'] >= 50.0].head(10)
    
    msg = f"🏛️ <b>BIST KURUMSAL EMİR AKIŞI RAPORU</b>\n"
    msg += f"🗓 <i>Tarih: {datetime.now().strftime('%Y-%m-%d %H:%M')}</i>\n"
    msg += "<i>(İndikatörsüz Saf Mikro-Yapı ve Kurumsal Süpürme Tespiti)</i>\n\n"
    
    if sweepers.empty:
        msg += "⚠️ <i>Bugün net bir kurumsal süpürme veya likidite boşluğu tespit edilemedi.</i>"
        return msg

    msg += "🚀 <b>KURUMSAL SÜPÜRME RADARI (Top 10)</b>\n"
    for idx, row in sweepers.iterrows():
        ...
        msg += f"  └ <i>Süpürme: %{row['sweep_ratio']:.1f} | Hacim Z: +{row['vol_z']:.1f}σ | {row['regime']}</i>\n"
        
    return msg

def main():
    print(f"[{datetime.now().strftime('%H:%M:%S')}] === Institutional Order Flow Scanner Başlıyor ===")
