...
def _prepare(df: pd.DataFrame) -> pd.DataFrame:
    if df is None or df.empty:
        return pd.DataFrame()
    out = df.copy()
    if tarih not in out.columns or meta_score not in out.columns:
        return pd.DataFrame()
    out[tarih] = pd.to_datetime(out[tarih], errors="coerce").dt.normalize()
    out[meta_score] = pd.to_numeric(out[meta_score], errors="coerce")
    out = out.dropna(subset=[tarih, meta_score]).sort_values(tarih).copy()
    return out


def _wins_mask(df: pd.DataFrame) -> pd.Series:
    if ret_t3 in df.columns:
        r = pd.to_numeric(df[ret_t3], errors="coerce")
...
def _resolved_mask(df: pd.DataFrame) -> pd.Series:
    if ret_t3 in df.columns:
        return pd.to_numeric(df[ret_t3], errors="coerce").notna()
    if outcome in df.columns:
        s = df[outcome].astype(str).str.upper()
...
def _metrics(df: pd.DataFrame, threshold: float) -> Dict:
    ...
    sel = df[pd.to_numeric(df[meta_score], errors="coerce") >= float(threshold)].copy()
    ...
    if ret_t3 in sel.columns:
        ret = pd.to_numeric(sel[ret_t3], errors="coerce").dropna()
        if not ret.empty:
...
def optimize_win_rate(state: Dict, data: pd.DataFrame, *, current_threshold: Optional[float] = None) -> Dict:
    ...
    wo["score_col"] = meta_score
    wo["return_col"] = ret_t3 if ret_t3 in (data.columns if isinstance(data, pd.DataFrame) else []) else None
